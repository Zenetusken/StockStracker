...
function BottomPerformers({ holdings, limit = 5 }) {
  // Calculate and sort holdings by gain percentage (ascending - worst first)
  const bottomPerformers = useMemo(() => {
    ...
    // Sort by gain percentage ascending (worst performers first)
  ...
  if (bottomPerformers.length === 0) {
    return (
      <div className="bg-card rounded-lg border border-line p-6" data-testid="bottom-performers">
        <div className="flex items-center gap-2 mb-4">
          <TrendingDown className="w-5 h-5 text-loss" />
          <h3 className="text-lg font-semibold text-text-primary">Bottom Performers</h3>
        </div>
  ...
  return (
    <div className="bg-card rounded-lg border border-line p-6" data-testid="bottom-performers">
      ...
      <div className="overflow-x-auto">
        <table className="w-full" data-testid="bottom-performers-table">
          <thead>
          ...
          <tbody>
            {bottomPerformers.map((holding, index) => (
              <tr
                ...
                data-testid={`bottom-performer-row-${index}`}
              >
